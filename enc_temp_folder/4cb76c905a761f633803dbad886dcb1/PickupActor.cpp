...
{
	Super::BeginPlay();
	CurrentTime = 0; //Reset time alive
...
float APickupActor::TimeAlive()
{
	return CurrentTime;
}

bool APickupActor::IsPickedUp()
{
	return isPickedUp;
}
...
void APickupActor::Tick(float DeltaTime)
{
	...
	CurrentTime += DeltaTime; //Total Time Alive
	OnPickupTick(DeltaTime,CurrentTime); //Send time alive to Blueprint
}
